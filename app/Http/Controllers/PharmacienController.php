...
class PharmacienController extends Controller
{
    ...
    public function index()
    {
        ...
        ])->get(env('API_BASE_URL') . '/user/getPharmacyUsers');

        if ($response->status() == 200) {
            $utilisateurs = collect($response->json());

            $admins = $utilisateurs->where('pharmacyId', '==', session('user_data')['wallet']['pharmacyId'])->values();

            return view('users.pharmacien', compact('admins'));
        } else {
            // Gérer l'erreur
    ...
    public function store(Request $request)
    {
        ...
        $response = Http::withOptions([
            ...
            'email' => $request->email,
            'phoneNumber' => $request->phone ?? '00225' . $request->phone,
    ...
    public function profile(Request $request)
    {
        ...
        $customMessages = [
            'password.required' => "Veuillez saisir l'adresse email de l'utilisateur'.",
            'cpassword.required' => "Veuillez saisir le nom de l'utilisateur.",
        ];
        ...
        $response = Http::withOptions([
            ...
            'Accept' => 'application/json',
            'Content-Type' => 'application/json',
        ])->post(env('API_BASE_URL') . '/user/changePassword', [
            'username' => session('user_data')['username'],
            'lastPassword' => $request->password,
            'newPassword' => $request->cpassword,
        ]);
