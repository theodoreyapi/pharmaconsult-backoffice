...
class RequetesController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($response->status() == 200) {
            ...
            $requetes = $collecte->where('status', '==', 'EN_ATTENTE')->values();

            return view('pharmacies.requete', compact('requetes'));
        } else {
            // Gérer l'erreur
    ...
    public function update(Request $request, string $id)
    {
        ...
        $response = Http::withOptions([
            ...
            'Accept' => 'application/json',
            'Content-Type' => 'application/json',
        ])->post(env('API_BASE_URL_PHARMA') . '/requests-medicament/respond?requestId=' . $id . '&pharmacyId=' . session('user_data')['wallet']['pharmacyId'] . '&available=' . $request->statut);

        if ($response->status() == 200) {
            return back()->with('succes',  "Votre reponse a été bien enregistrée ");
        } else {
            return back()->withErrors(["Impossible d'enregistrer. Veuillez réessayer!!"]);
        }
    }
